...
class Player extends SessionObject {
  ...
  collectCoins(coins, io, levels) {
    ...
    touching.forEach((coin) => {
      ...
      this.updateValues();
      io.to(this.session.room).emit("collected", coin.id, this.id, true);
    });
  ...
  checkCollisions(coins, chests, io) {
          ...
          if (this.hittingPlayer(enemy)) {
            ...
            if (Date.now() - enemy.joinTime >= 5000) {
              ...
              if (enemy.health <= 0) {
                ...
                if (!enemy.ai && this.session.connectedToDatabase && !(this.session.players.deadPlayers.includes(enemy))) {
                  ...
                  .catch(e => {
                    console.error("DATABASE IS BROKEN LOL");
                  });
                }
                ...
                if (!enemy.ai && socketById) {
                  ...
                  socketById.to(this.session.room).emit("playerDied", enemy.id, {
                    killedBy: {
                      id: this.id,
                      name: this.name
                    },
                  ...
                  io.to(this.session.room).emit("playerDied", enemy.id, {
                    killedBy: {
                      id: this.id,
                      name: this.name
                    },
                ...
                io.to(this.session.room).emit("coin", drop, [enemy.pos.x, enemy.pos.y]);
    ...
    chests.forEach((chest) => {
      if (this.hittingChest(chest)) {
        ...
        io.to(this.session.room).emit("collected", chest.id, this.id, false);

        //drop coins at that spot
        var drop = chest.open();

        io.to(this.session.room).emit("coin", drop, [chest.pos.x + (chest.width / 2), chest.pos.y + (chest.height / 2)]);
        coins.push(...drop);
      }
    });
    return [coins, chests];
