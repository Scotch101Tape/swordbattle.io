...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.loadtext = this.add.text(this.canvas.width / 2, this.canvas.height / 2, "Connecting...", {
      fontFamily: "Arial",
      fontSize: "32px",
      color: "#ffffff"
    }).setOrigin(0.5).setScrollFactor(0, 0).setDepth(200);
    this.ping = 0;
  ...
  create() {
    ...
    //recaptcha
    grecaptcha.ready(() => {
      grecaptcha.execute(CAPTCHASITE, {
        action: "join"
      }).then((thetoken) => {
        ...
        this.miniMap = {
          people: [],
          scaleFactor: convert(1189, 96, this.canvas.width),
          square: undefined
        };
        ...
        //lvl text
        try {
          this.lvlText = this.add.text(this.canvas.width / 2, this.canvas.height / 5, "", {
            fontFamily: "Georgia, \"Goudy Bookletter 1911\", Times, serif"
          }).setFontSize(convert(1366, 75, this.canvas.width)).setDepth(75).setAlpha(0).setOrigin(0.5);
          this.lvlTextTween = undefined;

          this.lvlState = this.add.text(this.canvas.width / 2, this.lvlBar.y - (this.lvlBar.height), "", {
            fontFamily: "Georgia, \"Goudy Bookletter 1911\", Times, serif"
          }).setFontSize(convert(1366, 50, this.canvas.width)).setDepth(75).setAlpha(1).setOrigin(0.5);
        ...
        const resize = () => {
          ...
          try {
            ...
            if (this.spectating) {
              ...
              this.deadText = this.add.text(this.canvas.width / 2, (this.deathRect.y - (this.deathRect.height / 2)), "You got stabbed", {
                fontFamily: "Arial",
                fontSize: "32px",
                color: "#000000"
              }).setOrigin(0.5);
              ...
              this.dataText = this.add.text(this.canvas.width / 2, this.deadText.y, msg, {
                fontFamily: "Arial",
                fontSize: "32px",
                color: "#000000"
              }).setOrigin(0.5);
        ...
        this.socket.on("collected", (coinId, playerId, coin) => {
          ...
          if (this.coins.find(coin => coin.id == coinId)) {
						...
						this.coins.find(coin => coin.id == coinId).state = {
							collected: true,
  ...
  update(time, delta) {
    ...
    this.background.setTilePosition(
      ((this.cameras.main.scrollX * this.cameras.main.zoom) + (player.x - (this.cameras.main.scrollX * this.cameras.main.zoom) - (this.canvas.width / 2))), ((this.cameras.main.scrollY * this.cameras.main.zoom) + (player.y - (this.cameras.main.scrollY * this.cameras.main.zoom) - (this.canvas.height / 2)))
    );
